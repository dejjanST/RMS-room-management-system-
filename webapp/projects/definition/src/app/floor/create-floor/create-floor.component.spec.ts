...
describe('CreateFloorComponent', () => {
  ...
  beforeEach(async(() => {
    ...
    const floorSpy = jasmine.createSpyObj('FloorService', ['create']);
  ...
  beforeEach(() => {
    ...
    floorService.create.and.returnValue(of({ msg: 'success' }));
  ...
  it('Floor Layout select validation', () => {
    const floorLayout = component.form.controls.floorLayout;
  ...
  it('Floor name validation', () => {
    const floorName = component.form.controls.floorName;
    floorName.setValue('');
    expect(floorName.hasError('required')).toBeTruthy();
    expect(floorName.valid).toBeFalsy();

    floorName.setValue('a');
    expect(floorName.hasError('minlength')).toBeTruthy();
    expect(floorName.valid).toBeFalsy();

    floorName.setValue('Floor 4');
    expect(floorName.hasError('minlength')).toBeFalsy();
    expect(floorName.valid).toBeTruthy();
  });


  it('Floor number validation', () => {
    const floorNumber = component.form.controls.floorNumber;
    floorNumber.setValue('');
    expect(floorNumber.hasError('required')).toBeTruthy();
    expect(floorNumber.valid).toBeFalsy();

    floorNumber.setValue('a');
    expect(floorNumber.hasError('pattern')).toBeTruthy();
    expect(floorNumber.valid).toBeFalsy();

    floorNumber.setValue(2);
    expect(floorNumber.hasError('required')).toBeFalsy();
    expect(floorNumber.hasError('pattern')).toBeFalsy();
    expect(floorNumber.valid).toBeTruthy();

    // test if floor number is negative
    floorNumber.setValue(-2);
    expect(floorNumber.hasError('pattern')).toBeFalsy();
    expect(floorNumber.valid).toBeTruthy();
  });
  ...
  it('form is invalid, submit button is disabled', () => {
    component.form.controls.floorLayout.setValue('');
    component.form.controls.floorName.setValue('');
    component.form.controls.floorNumber.setValue('');
    fixture.detectChanges();
  ...
  it('form is valid, submit button is enabled and on click call createNewFloor method', () => {
    component.form.controls.floorLayout.setValue(1);
    component.form.controls.floorName.setValue('Floor Name 1');
    component.form.controls.floorNumber.setValue(2);
    ...
    expect(floorService.create).toHaveBeenCalledTimes(1);
  });

  it('fill the form with negative floor number and on click call createNewFloor method', () => {
    component.form.controls.floorLayout.setValue(1);
    component.form.controls.floorName.setValue('Floor Name 1');
    component.form.controls.floorNumber.setValue(-2);
...
describe('FloorComponent - Edit Floor', () => {
  let component: CreateFloorComponent;
  let fixture: ComponentFixture<CreateFloorComponent>;
  let floorLayoutService: any;
  let floorService: any;
  ...
  beforeEach(async(() => {
    const floorLayoutSpy = jasmine.createSpyObj('FloorLayoutService', ['getFloorLayoutsBySiteList', 'getFloorLayout']);
    const floorSpy = jasmine.createSpyObj('FloorService', ['update', 'get']);
  ...
  beforeEach(() => {
    floorLayoutService = TestBed.inject(FloorLayoutService);
    floorService = TestBed.inject(FloorService);
    floorService.get.and.returnValue(of(mockFloor));
    floorLayoutService.getFloorLayoutsBySiteList.and.returnValue(of(mockFloorLayoutList));
    floorService.update.and.returnValue(of({ msg: 'success' }));
  ...
  it('form is valid, submit button is enabled and on click call update method', () => {
    ...
    expect(floorService.update).toHaveBeenCalledTimes(1);
  });
});
